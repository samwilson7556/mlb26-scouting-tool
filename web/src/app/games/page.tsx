...
import { GameSummary, getGames } from "@/lib/api";
import { formatGameDateTime, getGameDateSortValue } from "@/lib/dates";
...
const MLBTS_USERNAME = "poopoopee155";
const MLBTS_PLATFORM = "psn";

const PAGE_SIZE_OPTIONS = ["all", "10", "25", "50", "100"] as const;
...
export default function GamesPage() {
  ...
  const [sortKey, setSortKey] = useState<SortKey>("display_date");
  ...
  const visibleGames = useMemo(() => {
    ...
    return sortedGames.slice(0, Number(pageSize));
  }, [sortedGames, pageSize]);
  ...
  function handleSort(nextSortKey: SortKey) {
    ...
    setSortDirection(nextSortKey === "display_date" ? "desc" : "asc");
  }
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-slate-400">
            Browse your filtered online game history, sort by any column, and
            open individual MLB The Show game pages.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4 border-b border-slate-800 bg-black px-5 py-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="text-sm font-black uppercase tracking-wide text-white">
              Showing {visibleGames.length} of {total} games
            </div>
            <div className="mt-1 text-xs text-slate-500">
              MLB-style compact table. Click any column header to sort.
            </div>
          </div>

          <label className="flex items-center gap-3 text-sm text-slate-300">
            <span className="font-bold uppercase tracking-wide">Rows</span>
            <select
              ...
              <tbody>
                {visibleGames.map((game) => (
                  <tr key={game.id}>
                    ...
                    <td className="primary">{game.home_full_name}</td>

                    <td className="primary">{game.away_full_name}</td>
                    ...
                    <td className="numeric">
                      {game.home_hits ?? "-"}-{game.away_hits ?? "-"}
                    </td>

                    <td className="numeric divider-left font-mono text-xs">
                      <a
                        href={buildGameUrl(game.id)}
                        target="_blank"
                        rel="noreferrer"
                        className="mlb-link"
  ...
  return (
    ...
    >
      <button
        ...
        className={`mlb-sort-button ${isActive ? "mlb-sort-active" : ""}`}
      >
        <span>{label}</span>
        <span>{arrow}</span>
      </button>
...
function getSortValue(game: GameSummary, sortKey: SortKey): string | number {
  switch (sortKey) {
    case "display_date":
      return getGameDateSortValue(game.display_date);

    case "opponent_name":
      return game.opponent_name || "";

    case "user_result":
      return game.user_result || "";

    case "home_full_name":
      return game.home_full_name || "";

    case "away_full_name":
      return game.away_full_name || "";

    case "score":
      return Number(game.home_runs || 0) + Number(game.away_runs || 0);

    case "hits":
      return Number(game.home_hits || 0) + Number(game.away_hits || 0);

    case "id":
      return Number(game.id || 0);
...
function buildGameUrl(gameId: string): string {
  const params = new URLSearchParams({
    platform: MLBTS_PLATFORM,
    username: MLBTS_USERNAME,
  });
